...
class MyTestCase(unittest.TestCase):
    ...
    def test_connected_component_node(self):
        ...
        self.assertTrue(list._contains_(g.get_all_v().get(5).get_key()))
        self.assertTrue(list._contains_(g.get_all_v().get(6).get_key()))
        # add 7 but only one sided
        g.add_edge(0, 7, 5)
        list = algo.connected_component(0)
        self.assertFalse(list._contains_(g.get_all_v().get(7)))
        ...
        self.assertTrue(list._contains_(g.get_all_v().get(8).get_key()))
        # ccs should only contain 9 (no edges from 9)
        list2 = algo.connected_component(9)
        self.assertFalse(list2._contains_(g.get_all_v().get(8).get_key()))
        self.assertTrue(list2._contains_(g.get_all_v().get(9).get_key()))
        ...
        for node in list3:
            self.assertTrue(list._contains_(node))
    ...
    def test_list_of_SCC(self):
        ...
        self.assertTrue(algo.connected_components()._contains_(algo.connected_component(4)))
        g.add_edge(0, 1, 1)
        self.assertEqual(5, len(algo.connected_components()))
        g.add_edge(1, 0, 1)  # SCC with 0 and 1
        self.assertEqual(4, len(algo.connected_components()))
        self.assertTrue(algo.connected_components()._contains_(algo.connected_component(0)))
        g.add_edge(1, 2, 0)
        self.assertEqual(4, len(algo.connected_components()))
        g.add_edge(2, 0, 0)
        ...
        self.assertEqual(4, len(algo.connected_components()))
        g.add_edge(2, 0, 0)
        self.assertTrue(algo.connected_components()._contains_(algo.connected_component(2)))
