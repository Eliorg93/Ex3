...
class DiGraph:
    ...
    def __init__(self):
        ...
        self.__neighbors_inPrint: Dict[int, float] = dict()
        self.__neighborsPrint: Dict[int, float] = dict()
    ...
    def add_edge(self, id1: int, id2: int, weight: float, ) -> bool:
        if id1 is not id2 and id1 in self.__nodes and id2 in self.__nodes and id2 not in self.__neighbors.get(id1):
            ...
            self.__neighborsPrint.update({id2: edge.get_weight()})
            self.__neighbors_in.get(id2).update({id1: edge})
            self.__neighbors_inPrint.update({id2: edge.get_weight()})
            self.__mc += 1
            self.__size_edges += 1
            return True
        else:
    ...
    def add_node(self, node_id: int, pos: tuple = None) -> bool:
        if node_id not in self.__nodes:
            ...
            self.__neighbors_inPrint.update({node_id: -1})
            self.__neighborsPrint.update({node_id: -1})
            self.__mc += 1
            return True
        else:
            return False
    ...
    def remove_edge(self, node_id1, node_id2: int) -> bool:
        if node_id1 is not node_id2 and node_id1 in self.__nodes and node_id2 in self.__nodes and node_id2 in self.__neighbors.get(
                node_id1):
            # remove edge
            self.__neighbors.get(node_id1).pop(node_id2)
            self.__neighborsPrint.pop(node_id2)
            self.__neighbors_in.get(node_id2).pop(node_id1)
            self.__neighbors_inPrint.pop(node_id1)
            self.__mc += 1
            self.__size_edges -= 1
            return True
        else:
    ...
    def __repr__(self):
        return "Graph: |V|= %s , |E|= %s " % (len(self.get_all_v()), self.e_size())
